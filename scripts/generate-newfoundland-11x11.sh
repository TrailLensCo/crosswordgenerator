...
# Script to generate Newfoundland 11x11 crossword puzzle with logging and analysis

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
SRC_DIR="$PROJECT_ROOT/src"
CONFIG_FILE="$PROJECT_ROOT/config/sample_configs/newfoundland-11x11.yaml"

echo "╔════════════════════════════════════════════════════════════════╗"
echo "║     Newfoundland 11x11 Crossword Generator with Analysis      ║"
echo "╚════════════════════════════════════════════════════════════════╝"
echo ""
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo ""
    echo "Output files are in: $PROJECT_ROOT/output/newfoundland-11x11/"
    echo ""
    echo "Generated files:"
    echo "  • Puzzle grid (blank): newfoundland_culture__11x11_puzzle.svg"
    echo "  • Clues list: newfoundland_culture__11x11_clues.svg"
    echo "  • Solution grid: newfoundland_culture__11x11_solution.svg"
    echo "  • Complete HTML: newfoundland_culture__11x11_complete.html"
    ...
    echo ""
    echo "Check the log file in $PROJECT_ROOT/output/newfoundland-11x11/ for details."
    exit $EXIT_CODE
fi
